...
db.once("open", function() {
  ...
  app.get("/api/users/mayorque", async (req, res) => {
    const users = await base.find({ edad: { $gt: 30 } });
    res.json(users);
  });
  ...
  app.get("/api/users/mayorigual", async (req, res) => {
    const users = await base.find({ edad: { $gte: 30 } });
    res.json(users);
  });
  ...
  app.get("/api/users/coicidencdfssfdias", async (req, res) => {
    const users = await base.find({ edad: { $nin: [5, 10, 25] } });
    res.json(users);
  });
